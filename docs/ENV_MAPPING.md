...
### ② 生产环境（Docker Compose 部署在 192.168.26.66）
...
| 变量 | 实际值 | 对应宿主机 |
|---|---|---|
| `MARKET_API` | `http://host.docker.internal:8882` | `192.168.26.66:8882` |
| `TRADING_EXTERNAL_API` | `http://host.docker.internal:8882` | `192.168.26.66:8882` |
| `TRADING_TRADER_API` | `http://host.docker.internal:8003` | `192.168.26.66:8003` |
| `TRADING_ORDER_API` | `http://host.docker.internal:8881` | `192.168.26.66:8881` |
| `TRADING_RECORD_API` | `http://host.docker.internal:8881` | `192.168.26.66:8881` |
| `USER_API` | `http://host.docker.internal:8001` | `192.168.26.66:8001` |
...
| 服务 | 开发环境 | 生产环境（Docker → 宿主机 192.168.26.66） |
|---|---|---|
| 前端开发服务器 | `192.168.66.186:3000` | — |
| 后端 FastAPI | `192.168.66.186:8766` | Docker `backend:8766` → 暴露 `:8766` |
| Nginx 入口 | — | Docker `web:80` → 暴露 `:80` |
| PostgreSQL | 宿主机 `localhost:5432` | Docker 内部 `postgres:5432` |
| **行情数据 (MARKET)** | `192.168.66.143:8099` | `192.168.26.66:8882` |
| **策略/交易外部 (EXTERNAL)** | `192.168.66.143:8000` | `192.168.26.66:8882`（合并到 8882） |
| **Trader** | `192.168.66.155:8003` | `192.168.26.66:8003` |
| **订单 (ORDER)** | `192.168.66.135:8000` | `192.168.26.66:8881` |
| **交易记录 (RECORD)** | `192.168.66.135:8001` | `192.168.26.66:8881`（合并到 8881） |
| **用户 (USER)** | `192.168.66.198:8001` | `192.168.26.66:8001` |
...
| 服务 | 开发 IP | 生产 IP | 备注 |
|---|---|---|---|
| 所有上游服务 | 多个分散 IP | 统一 `192.168.26.66` | 生产环境将服务集中部署到一台宿主机 |
