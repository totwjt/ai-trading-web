...
#### 已完成：
1. ✅ 新增 `/macro-calendar` 页面
   - 按 `design/ui/stitch/宏观日历` 设计稿补齐左侧筛选、时间切换、经济指标表格与洞察卡片
   - 保持为 `MainLayout` 下的 main 内容区域

2. ✅ 新增 `/factor-board` 页面
   - 按 `design/ui/stitch/因子管理` 设计稿补齐标题区、因子归因表、搜索栏与右下角洞察面板
   - 页面名称按设计语义采用“因子看板”
...
### 3.4 实盘模拟补充：交易终端挂单字段扩展（2026-04-09） ✅

#### 已完成：
1. ✅ 为 `pending_orders` 设计补充挂单价格、挂单数量字段
2. ✅ 扩展交易终端挂单接口，支持价格、数量的查询、新增与更新
3. ✅ 更新 `/trading-terminal` 挂单列表，展示并支持编辑挂单价格、挂单数量
